...
export async function getAssetRows(csvUrl: string): Promise<AssetRow[]> {
  ...
  return parsed.map(row => {
    return {
      ...
      childrenAsset: parseEmptyStringToUndefined(row[11]) ? parseToNumber(row[11]) : undefined,
      ownDebt: parseToNumber(row[12]),
      spouseDebt: parseEmptyStringToUndefined(row[13]) ? parseToNumber(row[13]) : undefined,
      childrenDebt: parseEmptyStringToUndefined(row[14]) ? parseToNumber(row[14]) : undefined,
      ownIncome: parseToNumber(row[15]),
      spouseIncome: parseEmptyStringToUndefined(row[16]) ? parseToNumber(row[16]) : undefined,
      childrenIncome: parseEmptyStringToUndefined(row[17]) ? parseToNumber(row[17]) : undefined,
      ownExpense: parseToNumber(row[18]),
      spouseExpense: parseEmptyStringToUndefined(row[19]) ? parseToNumber(row[19]) : undefined,
      childrenExpense: parseEmptyStringToUndefined(row[20]) ? parseToNumber(row[20]) : undefined,
      ownTaxed: parseToNumber(row[21]),
      spouseTaxed: parseEmptyStringToUndefined(row[22]) ? parseToNumber(row[22]) : undefined,
      fillingUrl: row[23],
      remark: parseEmptyStringToUndefined(row[24]),
    }
  })
}
